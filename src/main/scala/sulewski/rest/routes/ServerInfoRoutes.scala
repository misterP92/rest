...
object ServerInfoRoutes {
  ...
  def apply(underlyingLogic: ActorRef[ServerInfoApi.ServerInfoBaseCommand])(implicit ec: ExecutionContext, system: ActorSystem[RootCommands]): ServerInfoRoutes = new ServerInfoRoutes(underlyingLogic)
}

class ServerInfoRoutes(underlyingLogic: ActorRef[ServerInfoApi.ServerInfoBaseCommand])
                      (implicit ec: ExecutionContext, system: ActorSystem[RootCommands]) extends Router with Directives {
